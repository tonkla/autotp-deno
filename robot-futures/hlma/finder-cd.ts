...
const Finder = ({ config, symbols, db, redis, exchange }: ExtBotProps) => {
  ...
  async function createLongLimit() {
    ...
    for (const symbol of symbols) {
      ...
      if (tad.hl_0 < 0.2) continue
      if (tad.hc_0 > 0.2) continue
      if (tad.hsl_0 < 0) continue
      if (tad.lsl_0 < 0.05) continue
      ...
      const _gap = config.orderGapAtr * tad.atr
  ...
  async function cancelTimedOut() {
    ...
    for (const o of orders) {
      ...
      if (Math.abs(markPrice - o.openPrice) < 0.1 * tad.atr) continue
