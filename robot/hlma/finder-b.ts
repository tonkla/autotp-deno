...
const Finder = ({ config, symbols, db, redis, exchange }: ExtBotProps) => {
  ...
  async function createLongLimit() {
    ...
    for (const symbol of symbols) {
      ...
      if (tah.hsl_0 < -0.1 && tah.lsl_0 < 0) continue
  ...
  async function createShortLimit() {
    ...
    for (const symbol of symbols) {
      ...
      if (tah.hsl_0 > 0 && tah.lsl_0 > 0.1) continue
      if (markPrice < tah.cma_0 + tah.atr * 0.3) continue
  ...
  async function createLongStop() {
    ...
    for (const o of orders) {
      ...
      const shouldSl = tah.lsl_0 < 0.02 && minutesToNow(o.openTime) > 5
  ...
  async function createShortStop() {
    ...
    for (const o of orders) {
      ...
      const shouldSl = tah.hsl_0 > -0.02 && minutesToNow(o.openTime) > 5
      const slMin = tah.atr * config.slMinAtr
...
const FinderB: BotFunc = async ({ symbols, db, redis, exchange }: BotProps) => {
  ...
  const bots: Config[] = [
    ...
    { ...cfg, botId: 'BH', maTimeframe: Interval.H12 },
    { ...cfg, botId: 'BD', maTimeframe: Interval.D1 },
  ]
