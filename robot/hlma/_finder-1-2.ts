...
class FinderD1 implements BotClass {
  private symbols
  private db
  private redis
  private exchange
  ...
  private async prepare(symbol: string): Promise<Prepare | null> {
    const _tad = await this.redis.get(RedisKeys.TA(config.exchange, symbol, config.maTimeframe))
    if (!_tad) return null
